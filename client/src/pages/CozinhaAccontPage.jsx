      ...
      <div className="bg-purple-100 p-6 rounded-lg shadow-lg w-full max-w-md">
        <div className="flex flex-col items-center mb-6">
          ...
          <p className="text-xl text-gray-600 text-center mb-4">{
                user.notas && user.notas.length > 0 ? `Avaliação: ${(user.notas.reduce((acc, nota) => acc + nota, 0) /user.notas.length).toFixed(2)}`
                : "Sem Avaliações"}</p>
        ...
        <div className="space-y-4 text-left">
          ...
          <div className="flex justify-between" >
            <span className="font-semibold text-gray-600">Estados atuante:</span>
            <span>{Array.isArray(user.estados) ? user.estados.join(", ") : "Nenhum estado disponível"}</span>
          </div>
          <div>
